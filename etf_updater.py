"""
etf_updater.py — به‌روزرسانی قیمت صندوق‌ها
...
def update_etf_prices(start_date, end_date=None, full=False, only=None):
    """Update every ETF (or just `only`) in one process, sequentially.

    This is the legacy path, kept for `python etf_updater.py` and for a machine with no
...
if __name__ == "__main__":
    ...
    args = sys.argv[1:]
    if not args:
        print("usage: python etf_updater.py <start_jalali> [end_jalali] [full]")
        raise SystemExit(2)
    update_etf_prices(args[0], args[1] if len(args) > 1 else None,
           full=("full" in args))
